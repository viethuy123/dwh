...
from utils.mappings import fact_dtm_mapping
...
dag = DAG(
    ...
    schedule='0 3 * * *',  # 3 AM - 1 tiếng sau dimensions
...
# Sensor: Đợi dimensions DAG hoàn thành
wait_for_dimensions = ExternalTaskSensor(
    task_id='wait_for_dimensions',
    external_dag_id='dimensions',
    external_task_id='end',  # Đợi task 'end' của dimensions DAG
    ...
    execution_delta=timedelta(hours=1),  # Dimensions chạy trước 1 tiếng
    dag=dag
)
...
def data_notification(tgt_table: str, **kwargs) -> None:
    result = kwargs['ti'].xcom_pull(
        task_ids=f'facts_group.dtm_fact_{tgt_table}.dtm_fact-{tgt_table}_quality_check', 
        key=f'dtm_fact-{tgt_table}_validation_results'
    )
    total_rows = extract_sql_data(pg_uri, f"SELECT count(*) as total_rows FROM dtm.fact.{tgt_table}")['total_rows'][0]
    prev_rows = int(Variable.get(f"dtm_fact-{tgt_table}_prev_rows", 0))
    new_rows_inserted = total_rows - prev_rows
    
    send_validation_results(
        table_name=f'dtm_fact-{tgt_table}', 
    ...
    Variable.set(f"dtm_fact-{tgt_table}_prev_rows", str(total_rows))
    return None
...
def save_job_logs(src_table: str | list, tgt_table: str, status: str, **context) -> None:
    ...
    save_etl_job_logs(
        ...
        target_table=f"dtm.fact.{tgt_table}",
...
with TaskGroup(group_id='facts_group', dag=dag) as fact_group:
    
    for tgt_table, src_table in fact_dtm_mapping.items():
        
        with TaskGroup(group_id=f'dtm_fact_{tgt_table}', dag=dag) as inner_group:
            ...
            dbt_run_dtm = DbtRunOperator(
                ...
                select=[f"path:models/dtm/fact/{tgt_table}.sql"],  # Đổi path
                target="dtm",  
                profile="dwh_project",
            ...
            data_notification_task = PythonOperator(
                dag=dag,
                task_id=f'dtm_fact-{tgt_table}_notification',
            ...
            success_save_logs_task = PythonOperator(
                dag=dag,
                task_id=f'success_save_logs_dtm_fact-{tgt_table}',
            ...
            failure_save_logs_task = PythonOperator(
                dag=dag,
                task_id=f'failure_save_logs_dtm_fact-{tgt_table}',
                python_callable=save_job_logs,
                op_kwargs={
                    'src_table': src_table,
                    'tgt_table': tgt_table,
...
start >> wait_for_dimensions >> fact_group >> end