...
from utils.mappings import jira_dtm_mapping
...
def data_quality_check(tgt_table:str,**kwargs) -> None:
    ...
    target_data = extract_sql_data(pg_uri, f"SELECT * FROM jira.{tgt_table} LIMIT 1000")
 
    result = validate_dataframe(df=target_data, suite_name=f"jira_dtm-{tgt_table}")

    kwargs['ti'].xcom_push(
        key=f'jira_dtm-{tgt_table}_validation_results',
        value=result
    )
...
def data_notification(tgt_table:str, **kwargs) -> None:
    ...
    result = kwargs['ti'].xcom_pull(
        task_ids=f'warehouse_to_mart.jira_dtm_{tgt_table}.jira_dtm-{tgt_table}_quality_check', 
        key=f'jira_dtm-{tgt_table}_validation_results')
    total_rows = extract_sql_data(pg_uri, f"SELECT count(*) as total_rows FROM jira.{tgt_table}")['total_rows'][0]
    prev_rows = int(Variable.get(f"jira_dtm-{tgt_table}_prev_rows",0))
    new_rows_inserted = total_rows - prev_rows
    send_validation_results(
        table_name=f'jira_dtm-{tgt_table}', 
        validation_result=result, 
        slack_channel_id=slack_chat_id, 
        slack_bot_token=slack_bot_token,
        total_rows=total_rows, 
        new_rows_inserted=new_rows_inserted)
    Variable.set(f"jira_dtm-{tgt_table}_prev_rows",str(total_rows))
...
def save_job_logs(src_table:str | list, tgt_table:str, status:str, **context) -> None:
    ...
    save_etl_job_logs(
        ...
        target_table=f"jira_dtm-{tgt_table}",
...
with TaskGroup(group_id='warehouse_to_mart', dag=dag) as outer_group:
    ...
    for tgt_table, src_table in jira_dtm_mapping.items():
        ...
        with TaskGroup(group_id = f'jira_dtm_{tgt_table}', dag=dag) as inner_group:
            ...
            data_quality_task = PythonOperator(
                    dag=dag,
                    task_id=f'jira_dtm-{tgt_table}_quality_check',
            ...
            data_notification_task = PythonOperator(
                    dag=dag,
                    task_id=f'jira_dtm-{tgt_table}_notification',
            ...
            success_save_logs_task = PythonOperator(
                    dag=dag,
                    task_id=f'success_save_logs_jira_dtm-{tgt_table}',
            ...
            failure_save_logs_task = PythonOperator(
                    dag=dag,
                    task_id=f'failure_save_logs_jira_dtm-{tgt_table}',
                    python_callable=save_job_logs,
                    op_kwargs={
                        'src_table':src_table,
                        'tgt_table':tgt_table,
