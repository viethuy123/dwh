...
from utils.mappings import dimension_dtm_mapping
...
def data_notification(tgt_table: str, **kwargs) -> None:
    result = kwargs['ti'].xcom_pull(
        task_ids=f'dimensions_group.dtm_dim_{tgt_table}.dtm_dim-{tgt_table}_quality_check', 
        key=f'dtm_dim-{tgt_table}_validation_results'
    )
    total_rows = extract_sql_data(pg_uri, f"SELECT count(*) as total_rows FROM dtm.dimension.{tgt_table}")['total_rows'][0]
    prev_rows = int(Variable.get(f"dtm_dim-{tgt_table}_prev_rows", 0))
    new_rows_inserted = total_rows - prev_rows
    
    send_validation_results(
        table_name=f'dtm_dim-{tgt_table}', 
    ...
    Variable.set(f"dtm_dim-{tgt_table}_prev_rows", str(total_rows))
    return None
...
def save_job_logs(src_table: str | list, tgt_table: str, status: str, **context) -> None:
    ...
    save_etl_job_logs(
        ...
        target_table=f"dtm.dimension.{tgt_table}",
...
with TaskGroup(group_id='dimensions_group', dag=dag) as dim_group:
    
    for tgt_table, src_table in dimension_dtm_mapping.items():
        ...
        with TaskGroup(group_id=f'dtm_dim_{tgt_table}', dag=dag) as inner_group:
            ...
            dbt_run_dtm = DbtRunOperator(
                ...
                select=[f"path:models/dtm/dimension/{tgt_table}.sql"],
                target="dtm",  
                profile="dwh_project",
            ...
            data_notification_task = PythonOperator(
                dag=dag,
                task_id=f'jisseki_dtm-{tgt_table}_notification',
            ...
            success_save_logs_task = PythonOperator(
                dag=dag,
                task_id=f'success_save_logs_dtm_dim-{tgt_table}',
            ...
            failure_save_logs_task = PythonOperator(
                dag=dag,
                task_id=f'failure_save_logs_dtm_dim-{tgt_table}',
                python_callable=save_job_logs,
                op_kwargs={
                    'src_table': src_table,
                    'tgt_table': tgt_table,
